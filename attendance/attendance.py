...
# Commands
    ...
    @commands.command(name="attendance", aliases=["attend"], help="Lists all users in a voice channel and outputs them into a text channel. ## **Subcommands:**\n\n**attend** - outputs attendance.\n <voice> - Specify the voice channel in quotes\n [role] - Optional - Limits the attendance to only those people in the specified role.\n [channel] - Optional - Outputs attendance to a specific channel, if not specified then the default channel is used.\n [per_page] - Optional specifies the amount of people to list per page This is a dynamic list with pagination.  Specify 0 here to create a list of all attendees that will not be paginated and can be reviewed later\n**attendchan** - sets the default text-channel for output is no channel is specified")
